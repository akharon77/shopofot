...
void Widget::draw(TransformStack &stack, RenderTarget &target)
{
    static plug::VertexArray vertices(PrimitiveType::LinesStrip, 5);
...
void Widget::draw(TransformStack &stack, RenderTarget &target)
{
    static VertexArray vertices(PrimitiveType::LinesStrip, 5);
    ...
    for (size_t id = 0; id < vertices.getSize(); ++id)
        vertices[id].color = Color(255, 0, 255, 255);
...
void Widget::onEvent(const Event &event, EHC &context)
{
    switch (event.getType())
    {
        case EventType::Tick: 
            {
                onTick((const TickEvent&) event, context);
            } 
            break;

        case EventType::KeyboardPressed: 
            {
                onKeyboardPressed((const KeyboardPressedEvent&) event, context);
            }
            break;

        case EventType::KeyboardReleased: 
            {
                onKeyboardReleased((const KeyboardReleasedEvent&) event, context);
            } 
            break;

        case EventType::MousePressed: 
            {
                onMousePressed((const MousePressedEvent&) event, context);
            }
            break;

        case EventType::MouseReleased: 
            {
                onMouseReleased((const MouseReleasedEvent&) event, context);
            }
            break;

        case plug::EventType::MouseMove:
            {
                onMouseMove((const MouseMoveEvent&) event, context);
            }
            break;
